...
const VehicleDetailDrawer: React.FC<Props> = ({
  vehicle,
  onClose,
  onDispatch,
}) => {
  ...
  return (
    <div className="absolute top-0 right-0 h-full w-80 md:w-96 bg-gov-800 border-l border-gov-700 shadow-gov-xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col">
      {/* Header */}
      <div className="p-5 border-b border-gov-700 flex justify-between items-center bg-gov-900/50 backdrop-blur">
        <div>
          ...
          <p className="text-sm text-gov-accent font-mono mt-0.5">
            {vehicle.model}
          </p>
        </div>
        <button
          onClick={onClose}
          className="p-2 hover:bg-gov-700 rounded-full text-gray-400 hover:text-white transition-colors"
        >
          <X size={20} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar">
        {/* Status Card */}
        <div className="bg-gov-900/80 rounded-xl p-5 border border-gov-700 shadow-gov-sm relative overflow-hidden">
          ...
          <div className="grid grid-cols-2 gap-6 relative z-10">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-gov-800 rounded-lg text-gray-400">
                <Gauge size={18} />
              </div>
              <div>
            ...
            <div className="bg-sky-500/5 border border-sky-500/20 p-4 rounded-xl flex items-center justify-between shadow-sm relative overflow-hidden group">
              ...
              <div className="flex items-center gap-4 text-sky-400 relative z-10">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-sky-400/70 uppercase font-bold tracking-wider">
                    Mission Active
                  </span>
                </div>
              </div>
              <div className="text-right relative z-10">
                ...
                <p className="font-mono text-white text-2xl font-bold leading-none tracking-tight">
                  {(vehicle.etaMinutes || 0).toFixed(0)}
            ...
            <div className="bg-amber-500/5 border border-amber-500/20 p-4 rounded-xl flex items-center justify-between shadow-sm">
              <div className="flex items-center gap-4 text-amber-500">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-amber-400/70 uppercase font-bold tracking-wider">
                    Idle Status
                  </span>
                </div>
              </div>
          ...
          <div className="bg-gov-900/50 rounded-lg p-3 border border-gov-700 space-y-3">
            <div className="flex items-center gap-3 text-sm text-gray-300">
              ...
              <span className="font-mono text-xs">
                Grid Sector {vehicle.location.x.toFixed(0)}-
                {vehicle.location.y.toFixed(0)}
              </span>
            </div>
            ...
            <div className="flex items-center gap-3 text-sm text-gray-300">
              <Radio
                size={16}
                className={`${vehicle.status !== VehicleStatus.OUT_OF_SERVICE ? "text-emerald-400 animate-pulse" : "text-red-500"}`}
              />
              <span className="text-xs">
                Last Signal:{" "}
                <span className="text-white font-bold">{vehicle.lastPing}</span>
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          {vehicle.assignedDriverId ? (
            <div className="flex items-center gap-4 bg-gov-700/20 p-3 rounded-xl border border-gov-700">
              <div className="w-10 h-10 rounded-full bg-gov-600 flex items-center justify-center text-xs font-bold text-white shadow-lg border border-gov-500">
                AS
              </div>
              <div>
                <p className="text-sm font-bold text-white">Agent Smith</p>
                <p className="text-[10px] uppercase tracking-wider text-emerald-400 font-bold bg-emerald-500/10 px-1.5 py-0.5 rounded w-fit mt-0.5 border border-emerald-500/20">
                  Active Duty
                </p>
              </div>
            </div>
