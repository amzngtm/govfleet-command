...
const StatCard = ({ title, value, icon: Icon, colorClass, subtext }: any) => (
  <div className="bg-gov-800 p-4 rounded-lg border border-gov-700 shadow-lg flex items-start justify-between hover:border-gov-600 transition-colors group">
    <div>
      <p className="text-gray-400 text-xs uppercase tracking-wider font-bold group-hover:text-gray-300 transition-colors">
        {title}
      </p>
    ...
    <div
      className={`p-3 rounded-md ${colorClass} bg-opacity-10 border border-opacity-20 border-current`}
    >
  ...
  return (
    <div className="bg-gov-800 p-4 rounded-lg border border-gov-700 shadow-lg hover:border-gov-600 transition-colors group flex flex-col justify-between">
      ...
      <div className="grid grid-cols-3 gap-2 text-center mt-2">
        ...
        <div className="bg-gov-900 rounded p-1 border border-gov-700">
          <span className="block text-amber-500 font-bold text-lg leading-none">
            {busy}
          </span>
          <span className="text-[10px] text-gray-500 uppercase">Busy</span>
        </div>
        <div className="bg-gov-900 rounded p-1 border border-gov-700">
          <span className="block text-gray-500 font-bold text-lg leading-none">
            {offline}
          </span>
...
const AuditLogFeed = ({
  ...
  <div className="bg-gov-800 rounded-lg border border-gov-700 flex flex-col h-full shadow-lg overflow-hidden min-h-50">
    ...
    <div className="flex-1 overflow-y-auto p-0 scrollbar-thin scrollbar-thumb-gov-700 scrollbar-track-gov-900">
      {logs.map((log, idx) => {
        ...
        return (
          <div
            key={log.id}
            className={`text-xs p-3 border-b border-gov-700/50 hover:bg-gov-700/30 transition-colors flex gap-3 ${idx === 0 ? "bg-emerald-500/5" : ""}`}
          >
            <div className="font-mono text-gray-500 text-[10px] w-14 shrink-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-gray-400 truncate" title={log.details}>
                {log.details}
              </div>
            </div>
          </div>
...
const ScheduledMissionsFeed = ({
  ...
  const upcoming = trips
    .filter(
      (t) =>
        t.status === TripStatus.SCHEDULED || t.status === TripStatus.DISPATCHED,
    )
    .sort(
      (a, b) =>
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2">
        {upcoming.length === 0 ? (
          ...
          upcoming.map((trip) => (
            <div
              key={trip.id}
              className="bg-gov-900/50 p-3 rounded border border-gov-700/50 hover:border-gov-500 transition-colors group"
            >
              <div className="flex justify-between items-center mb-1">
                <span className="text-gov-accent font-bold text-xs">
                  {new Date(trip.pickupTime).toLocaleTimeString([], {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
                </span>
                <span
                  className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${trip.priority === "VIP" ? "bg-amber-500/10 text-amber-500" : "bg-blue-500/10 text-blue-500"}`}
                >
                  {trip.priority}
                </span>
              </div>
              ...
              <p className="text-[10px] text-gray-500 flex items-center gap-1">
                <Car size={10} />{" "}
                {trip.vehicleId
                  ? vehicles.find((v) => v.id === trip.vehicleId)?.model
                  : "Unassigned"}
              </p>
            </div>
          ))
...
const IncomingRequestsFeed = ({
  ...
  return (
    <div className="bg-gov-800 rounded-lg border border-gov-700 flex flex-col h-full shadow-lg overflow-hidden min-h-50 relative">
      {/* Header */}
      <div className="p-3 border-b border-gov-700 flex justify-between items-center bg-gov-900/50 backdrop-blur shrink-0">
        <h3 className="text-xs font-bold text-gray-300 flex items-center gap-2 uppercase tracking-wider">
          <Bell size={14} className="text-gov-warning animate-pulse" /> Incoming
          Requests
        </h3>
        <span className="bg-gov-warning/20 text-gov-warning text-[10px] font-bold px-1.5 rounded">
          {requests.length}
        </span>
      </div>
      {/* List */}
      <div className="flex-1 overflow-y-auto p-0 scrollbar-thin scrollbar-thumb-gov-700">
        {requests.length === 0 ? (
          ...
          requests.map((req) => (
            <div
              key={req.id}
              className="p-3 border-b border-gov-700 hover:bg-gov-700/20 transition-colors group relative"
            >
              <div className="flex justify-between items-start mb-1">
                ...
                <span
                  className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${req.priority === "VIP" ? "bg-amber-500/20 text-amber-500" : "bg-blue-500/20 text-blue-500"}`}
                >
                  {req.priority}
                </span>
              </div>
              <div className="text-[10px] text-gray-400 space-y-1 pl-3.5">
                <div className="flex items-center gap-1">
                  <MapPin size={10} /> {req.pickupLocation}
                </div>
                <div className="flex items-center gap-1 ml-3.5 border-l border-gov-600 pl-2">
              ...
              <div
                className="mt-3 pl-3.5 flex gap-2"
                onClick={(e) => e.stopPropagation()}
              >
                <button
                  onClick={() => onAction(req.id, "APPROVE")}
                  className="flex-1 py-1 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 rounded text-[10px] font-bold flex items-center justify-center gap-1 transition-all active:scale-95 shadow-sm"
                >
                  <CheckSquare size={10} /> Approve
                </button>
                <button
                  onClick={() => onAction(req.id, "REJECT")}
                  className="flex-1 py-1 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 rounded text-[10px] font-bold flex items-center justify-center gap-1 transition-all active:scale-95 shadow-sm"
                >
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-white flex items-center gap-2">
          <CalendarIcon /> Operational Schedule
        </h2>
        <div className="flex gap-2">
          <button className="bg-gov-700 px-3 py-1 rounded text-xs text-white">
            Day
          </button>
          <button className="bg-gov-accent px-3 py-1 rounded text-xs text-white">
            Week
          </button>
          <button className="bg-gov-700 px-3 py-1 rounded text-xs text-white">
            Month
      ...
      <div className="bg-gov-800 border border-gov-700 rounded-lg overflow-hidden">
        <div className="grid grid-cols-7 border-b border-gov-700 bg-gov-900 text-center py-2">
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
            <div key={d} className="text-xs font-bold text-gray-500 uppercase">
              {d}
            </div>
          ))}
        </div>
        <div className="p-4 min-h-100">
          <div className="space-y-2">
            {sortedTrips.map((trip: any) => (
              <div
                key={trip.id}
                className="flex gap-4 p-3 bg-gov-900/50 rounded border border-gov-700 hover:border-gov-500 transition-colors"
              >
                <div className="text-center min-w-15 border-r border-gov-700 pr-4">
                  <div className="text-xs text-gov-accent font-bold uppercase">
                    {new Date(trip.pickupTime).toLocaleDateString("en-US", {
                      weekday: "short",
                    })}
                  </div>
                  <div className="text-2xl font-bold text-white">
                    {new Date(trip.pickupTime).getDate()}
                  </div>
                ...
                <div className="flex-1">
                  <div className="flex justify-between">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded text-[10px] font-bold ${trip.priority === "VIP" ? "bg-amber-500/20 text-amber-500" : "bg-blue-500/20 text-blue-500"}`}
                    >
                      {trip.priority}
                    </span>
                  </div>
                  <div className="flex items-center gap-4 mt-1 text-xs text-gray-400">
                    <span className="flex items-center gap-1">
                      <User size={12} /> {trip.passengerName}
                    </span>
                    <span className="flex items-center gap-1">
                      <Car size={12} />{" "}
                      {trip.vehicleId
                        ? vehicles.find((v: any) => v.id === trip.vehicleId)
                            ?.plate
                        : "Unassigned"}
                    </span>
                  </div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 h-full overflow-x-auto pb-4">
      {columns.map((status) => (
        <div
          key={status}
          className="bg-gov-800 rounded-lg border border-gov-700 flex flex-col min-h-100"
        >
          <div className="p-3 border-b border-gov-700 bg-gov-900/50 flex justify-between items-center">
            <h3 className="text-xs font-bold text-gray-300 uppercase tracking-wider">
              {status.replace("_", " ")}
            </h3>
            <span className="bg-gov-700 text-gray-300 text-[10px] px-1.5 rounded-full">
          ...
          <div className="p-2 space-y-2 flex-1 bg-gov-900/20">
            {orders
              ...
              .map((order: any) => (
                <div
                  key={order.id}
                  className="bg-gov-800 p-3 rounded border border-gov-600 shadow-sm hover:border-gov-accent cursor-pointer group relative"
                >
                  <div className="flex justify-between items-start mb-1">
                    <span className="text-[10px] text-gray-500 font-mono">
                      {order.id}
                    ...
                    <span
                      className={`w-2 h-2 rounded-full ${order.priority === "CRITICAL" ? "bg-red-500" : order.priority === "HIGH" ? "bg-orange-500" : "bg-blue-500"}`}
                    ></span>
                  </div>
                  ...
                  <div className="mt-2 pt-2 border-t border-gov-700/50 flex justify-between items-center">
                    ...
                    {order.assignedMechanicId && (
                      <div
                        className="w-5 h-5 rounded-full bg-gov-700 flex items-center justify-center text-[9px] text-white font-bold"
                        title={order.assignedMechanicId}
                      >
  ...
  return (
    <div className="bg-gov-800 border border-gov-700 rounded p-4 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between shadow-sm hover:border-gov-600 transition-all">
      <div className="flex items-start gap-4">
        <div
          className={`p-3 rounded-full shrink-0 ${incident.severity === "CRITICAL" ? "bg-red-500/20 text-red-500" : incident.severity === "HIGH" ? "bg-orange-500/20 text-orange-500" : "bg-blue-500/20 text-blue-500"}`}
        >
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span
              className={`text-[10px] font-bold px-2 py-0.5 rounded ${incident.status === "OPEN" ? "bg-red-500 text-white" : "bg-emerald-500 text-white"}`}
            >
              {incident.status}
            </span>
      ...
      <div className="flex items-center gap-2 w-full md:w-auto">
        {incident.status === "OPEN" && (
          <button
            ...
            className="flex-1 md:flex-none px-3 py-2 bg-gov-700 hover:bg-gov-600 text-white text-xs font-bold rounded"
          >
            Ack
          </button>
        )}
        ...
        <button
          ...
          className="flex-1 md:flex-none px-3 py-2 bg-gov-accent hover:bg-sky-500 text-white text-xs font-bold rounded flex items-center justify-center gap-2"
        >
...
const App: React.FC = () => {
  ...
  const [incomingRequests, setIncomingRequests] =
    useState<IncomingRequest[]>(MOCK_REQUESTS);
  const [auditLogs, setAuditLogs] = useState<AuditLogEntry[]>([
    {
      ...
      details: "Operational Command Center initialized.",
    },
    {
      id: "al-2",
      ...
      actorId: "disp1",
      action: "SHIFT_START",
      details: "Dispatcher Miller logged in.",
    },
  ]);
  ...
  const [mapStatusFilter, setMapStatusFilter] = useState<VehicleStatus | "ALL">(
    "ALL",
  );
  const [missionFilter, setMissionFilter] = useState<
    "ALL" | "ACTIVE" | "SCHEDULED" | "COMPLETED" | "CANCELLED" | "RESCHEDULED"
  >("ALL");
  ...
  const [assigningIncidentId, setAssigningIncidentId] = useState<string | null>(
    null,
  );

  // Simulation Mode
  const [viewMode, setViewMode] = useState<"ADMIN" | "DRIVER" | "RIDER">(
    "ADMIN",
  );
  ...
  const riderActiveTrip =
    viewMode === "RIDER"
      ? trips.find(
          (t) =>
            t.passengerName === currentUser.name &&
      ...
      setVehicles((prev) =>
        prev.map((v) => {
          ...
          if (
            ...
            v.status === VehicleStatus.ON_MISSION
          ) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      addAuditLog(
        "SYSTEM_ALERT",
        "New Mission Request received from Secret Service Detail",
      );
    }, 10000); // 10s delay
  ...
  const handleCreateTrip = (tripData: any) => {
    ...
    if (tripData.driverId) {
      setUsers((prev) =>
        prev.map((u) =>
          u.id === tripData.driverId ? { ...u, status: "BUSY" } : u,
        ),
      );
    }
    ...
    if (tripData.vehicleId) {
      setVehicles((prev) =>
        prev.map((v) =>
          v.id === tripData.vehicleId
            ? {
                ...v,
                status: VehicleStatus.ON_MISSION,
                etaMinutes: tripData.estimatedDurationMin,
              }
            : v,
        ),
      );
    ...
    addAuditLog(
      "DISPATCH_CREATED",
      `Trip ${tripData.id} created for vehicle ${tripData.vehicleId}`,
    );
  ...
  const handleAddVehicle = (vehicleData: Partial<Vehicle>) => {
    const newVehicle: Vehicle = {
      ...
      mileage: 0,
      nextServiceDate: new Date(Date.now() + 15778463000)
        .toISOString()
        .split("T")[0], // +6 months approx
      standbyTime: 0,
    };
    ...
    addAuditLog(
      "VEHICLE_ADDED",
      `New vehicle registered: ${newVehicle.plate} (${newVehicle.model})`,
    );
  };
  ...
  const handleTripUpdate = (
    ...
    setTrips((prev) =>
      prev.map((t) =>
        t.id === tripId ? { ...t, status: newStatus, feedback } : t,
      ),
    );
    ...
    if (
      ...
      if (trip.driverId) {
        setUsers((prev) =>
          prev.map((u) =>
            u.id === trip.driverId ? { ...u, status: "ONLINE" } : u,
          ),
        );
      }
      // Free up Vehicle
      if (trip.vehicleId) {
        setVehicles((prev) =>
          prev.map((v) =>
            v.id === trip.vehicleId
              ? { ...v, status: VehicleStatus.ACTIVE, etaMinutes: 0 }
              : v,
          ),
        );
      }
      ...
      if (feedback) {
        ...
        if (feedback.vehicleIssuesDetected) {
          addAuditLog(
            "TRIP_FEEDBACK",
            `Driver reported vehicle issues for Trip ${trip.id}. Creating maintenance flag.`,
          );
        }
      }
  ...
  const handleReportIncident = (data: {
    type: string;
    ...
    const newIncident: Incident = {
      ...
      category: data.type,
      description:
        data.notes || `Driver reported ${data.type} issue via mobile app.`,
      timestamp: new Date().toISOString(),
      status: "OPEN",
    };
    ...
    if (data.severity === IncidentSeverity.CRITICAL && vehicle) {
      ...
      addAuditLog(
        "CRITICAL_ALERT",
        `Vehicle ${vehicle.id} marked OUT_OF_SERVICE due to critical incident.`,
      );
    }

    addAuditLog(
      "INCIDENT_REPORTED",
      `Incident ${newIncident.id} reported by ${currentUser.name}`,
    );
  ...
  const handleChatRequest = (
    ...
    addAuditLog(
      "CHAT_REQUEST",
      `Chat requested by ${currentUser.name} [${urgency}]: ${topic}`,
    );
  ...
  const handleAnalyzeIncident = async (incident: Incident) => {
    ...
    addAuditLog(
      "AI_ANALYSIS",
      `AI Analysis requested for incident ${incident.id}`,
    );
  };
  ...
  const handleAcknowledgeIncident = (id: string) => {
    ...
    addAuditLog(
      "INCIDENT_ACK",
      `Incident ${id} acknowledged by ${currentUser.name}.`,
    );
  };
  ...
  const handleAgentAssignment = (agentId: string) => {
    ...
    addAuditLog(
      "INCIDENT_ASSIGNED",
      `Incident ${assigningIncidentId} assigned to ${agent?.name || agentId}`,
    );
  ...
  const handleUserUpdate = (userId: string, updates: Partial<UserProfile>) => {
    ...
    if (updates.role && user && updates.role !== user.role) {
      addAuditLog(
        "ROLE_CHANGE",
        `User ${user.name} role changed from ${user.role} to ${updates.role} by ${currentUser.name}`,
      );
    } else if (updates.codename) {
      addAuditLog(
        "PROFILE_UPDATE",
        `User ${user?.name} codename updated to "${updates.codename}"`,
      );
    } else {
  ...
  const handleRequestAction = (id: string, action: "APPROVE" | "REJECT") => {
    ...
    if (action === "REJECT") {
      if (
        !window.confirm(
          `Are you sure you want to reject the request from ${request.requestorName}?`,
        )
      )
        return;
      ...
      addAuditLog(
        "REQUEST_REJECTED",
        `Incoming request from ${request.requestorName} was rejected by ${currentUser.name}.`,
      );
    ...
    if (action === "APPROVE") {
      const newTrip: Trip = {
        ...
        notes: request.notes
          ...
          : "Generated from Incoming Request.",
      ...
      addAuditLog(
        "REQUEST_APPROVED",
        `Incoming request from ${request.requestorName} approved. Mission ${newTrip.id} scheduled.`,
      );
    }
  };
  ...
  const handleRiderRequest = (req: Partial<IncomingRequest>) => {
    ...
    addAuditLog(
      "RIDER_REQUEST",
      `Mobile Rider ${newReq.requestorName} requested a shuttle to ${newReq.dropoffLocation}`,
    );
  };
  ...
  const handleDeleteRoute = (id: string) => {
    if (confirm("Delete this route? This will unassign all vehicles.")) {
      ...
      setVehicles((prev) =>
        prev.map((v) =>
          v.assignedRouteId === id ? { ...v, assignedRouteId: undefined } : v,
        ),
      );
  ...
  const vehicleStatusData = [
    {
      name: "Active",
      value: vehicles.filter((v) => v.status === VehicleStatus.ACTIVE).length,
      color: "#10b981",
    },
    {
      name: "On Mission",
      value: vehicles.filter((v) => v.status === VehicleStatus.ON_MISSION)
        .length,
      color: "#0ea5e9",
    },
    {
      name: "Maintenance",
      value: vehicles.filter((v) => v.status === VehicleStatus.MAINTENANCE)
        .length,
      color: "#f59e0b",
    },
    {
      name: "Out of Service",
      value: vehicles.filter((v) => v.status === VehicleStatus.OUT_OF_SERVICE)
        .length,
      color: "#ef4444",
    },
  ];
  ...
  const incidentsData = [
    {
      name: "Crit",
      value: incidents.filter((i) => i.severity === IncidentSeverity.CRITICAL)
        .length,
    },
    {
      name: "High",
      value: incidents.filter((i) => i.severity === IncidentSeverity.HIGH)
        .length,
    },
    {
      name: "Med",
      value: incidents.filter((i) => i.severity === IncidentSeverity.MEDIUM)
        .length,
    },
    {
      name: "Low",
      value: incidents.filter((i) => i.severity === IncidentSeverity.LOW)
        .length,
    },
  ];
  ...
  const missionPriorityData = [
    {
      name: "VIP",
      value: trips.filter((t) => t.priority === "VIP").length,
      color: "#f59e0b",
    },
    {
      name: "Urgent",
      value: trips.filter((t) => t.priority === "URGENT").length,
      color: "#ef4444",
    },
    {
      name: "Standard",
      value: trips.filter((t) => t.priority === "STANDARD").length,
      color: "#3b82f6",
    },
  ];
  ...
  if (viewMode === "RIDER") {
    return (
      <div className="h-screen w-full bg-slate-200 flex items-center justify-center">
        <div className="w-full h-full md:h-200 md:w-100 md:rounded-3xl overflow-hidden border-8 border-slate-300 bg-white relative shadow-2xl">
          <RiderApp
            user={currentUser}
            vehicles={vehicles}
            users={users}
  ...
  if (viewMode === "DRIVER") {
    ...
    return (
      <div className="h-screen w-full bg-black flex items-center justify-center">
        <div className="w-full h-full md:h-200 md:w-100 md:rounded-3xl overflow-hidden border-8 border-gray-800 bg-gray-900 relative">
          <DriverApp
            user={currentUser}
  ...
  return (
    <div className="flex h-screen bg-gov-900 text-gray-100 overflow-hidden font-sans">
      {/* Sidebar */}
      <aside
        className={`${sidebarOpen ? "w-64" : "w-20"} bg-gov-800 border-r border-gov-700 flex flex-col transition-all duration-300 z-40`}
      >
        <div className="h-16 flex items-center justify-center border-b border-gov-700 bg-gov-900">
          {sidebarOpen ? (
            <div className="flex items-center gap-2 font-bold text-lg tracking-wider">
              <div className="w-8 h-8 bg-linear-to-br from-gov-accent to-blue-700 rounded flex items-center justify-center shadow-lg shadow-blue-900/50">
        ...
        <nav className="flex-1 py-6 space-y-1 px-3">
          ...
          <div className="pt-4 mt-4 border-t border-gov-700 space-y-1">
            <button
              ...
              className="w-full flex items-center gap-3 px-3 py-3 rounded text-emerald-400 hover:bg-emerald-500/10 transition-colors"
            >
              <Smartphone size={20} />
              {sidebarOpen && (
                <span className="font-medium text-sm">Switch App Mode</span>
              )}
            </button>
          </div>
        </nav>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        <header className="h-16 bg-gov-800/80 backdrop-blur border-b border-gov-700 flex items-center justify-between px-6 z-30 shadow-md">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="hidden md:flex bg-gov-800 hover:bg-gov-700 border border-gov-600 text-white px-3 py-2 rounded font-bold text-xs items-center gap-2 transition-all"
            >
              <HelpCircle size={14} /> Support
            </button>
            <button
              onClick={() => handleDispatchStart()}
              className="hidden md:flex bg-gov-accent hover:bg-sky-500 text-white px-4 py-2 rounded font-bold text-sm items-center gap-2 shadow-lg shadow-sky-900/20 transition-all hover:-translate-y-0.5"
            >
              <NavIcon size={16} /> New Dispatch
            </button>
            <div className="h-8 w-px bg-gov-700 mx-2"></div>
            <div className="flex items-center gap-3">
              ...
              <div className="w-9 h-9 rounded-full bg-linear-to-tr from-gov-700 to-gov-600 border border-gov-500 flex items-center justify-center text-xs font-bold text-white shadow-lg cursor-pointer hover:border-gov-accent transition-colors">
                AD
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gov-900">
            ...
            <div className="flex flex-col h-[calc(100vh-140px)] gap-6 overflow-y-auto pr-2 pb-20">
              ...
              <div className="shrink-0 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard
                  ...
                  value={`${vehicles.filter((v) => v.status === "ACTIVE" || v.status === "ON_MISSION").length}/${vehicles.length}`}
                  subtext="Vehicles Available"
                  icon={Car}
                ...
                <StatCard
                  ...
                  subtext="Real-time Ops"
                  icon={Map}
                  colorClass="text-sky-500"
                />
                <PersonnelStatCard users={users} />
                <StatCard
                  title="Crit Incidents"
                  value={
              ...
              <div className="shrink-0 grid grid-cols-1 lg:grid-cols-2 gap-6 min-h-62.5">
              ...
              <div className="shrink-0 h-150 flex flex-col bg-gov-800 rounded-lg border border-gov-700 shadow-xl overflow-hidden relative group">
                <div className="p-3 border-b border-gov-700 flex justify-between items-center bg-gov-900/50 backdrop-blur z-10 shrink-0">
                  <h3 className="text-sm font-bold text-gray-300 flex items-center gap-2 uppercase tracking-wider">
                    <Map size={16} className="text-gov-accent" /> Live Tactical
                    Feed
                  </h3>
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className="text-[10px] bg-gov-700 hover:bg-gov-600 px-2 py-0.5 rounded text-white transition-colors"
                    >
                      Expand
                    </button>
                  </div>
              ...
              <div className="shrink-0 min-h-50">
          ...
          {activeTab === "STATISTICS" && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                  <TrendingUp /> Operational Analytics
                </h2>
                <div className="flex gap-2">
                  <select className="bg-gov-800 border border-gov-600 text-xs rounded px-2 py-1 text-white">
                    <option>Last 7 Days</option>
                    <option>Last 30 Days</option>
                  </select>
                  <button className="bg-gov-accent px-3 py-1 rounded text-xs font-bold text-white hover:bg-sky-600">
                    Export Report
                  </button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                {/* Fleet Composition */}
                <div className="bg-gov-800 rounded-lg border border-gov-700 p-4 flex flex-col shadow-lg min-h-75">
                  <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 flex items-center gap-2">
                    <PieChartIcon size={14} /> Fleet Composition
                  </h3>
                  <div className="flex-1">
                ...
                <div className="bg-gov-800 rounded-lg border border-gov-700 p-4 flex flex-col shadow-lg min-h-75">
                  <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 flex items-center gap-2">
                    <BarChart2 size={14} /> Incident Severity
                  </h3>
                  <div className="flex-1">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                ...
                <div className="bg-gov-800 rounded-lg border border-gov-700 p-4 flex flex-col shadow-lg min-h-75">
                  <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 flex items-center gap-2">
                    <ClipboardList size={14} /> Mission Priorities
                  </h3>
                  <div className="flex-1">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                ...
                {/* Operational Volume */}
                <div className="lg:col-span-2 bg-gov-800 rounded-lg border border-gov-700 p-5 shadow-lg flex flex-col min-h-75">
                  <h3 className="text-sm font-bold text-white flex items-center gap-2 mb-4">
                    <Activity size={16} className="text-gov-accent" />{" "}
                    Operational Volume (Requests)
                  </h3>
                ...
                <div className="bg-gov-800 rounded-lg border border-gov-700 p-4 flex flex-col shadow-lg min-h-75">
                  <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 flex items-center gap-2">
                    <Fuel size={14} /> Fuel Consumption (Gal)
                  </h3>
                  <div className="flex-1">
                    <ResponsiveContainer width="100%" height="100%">
            ...
            <div className="h-full flex flex-col relative">
              <div className="absolute top-4 right-4 z-10 bg-gov-900/90 backdrop-blur border border-gov-700 p-2 rounded-lg shadow-xl flex flex-col gap-2">
                ...
                <div className="flex flex-col gap-1">
                  <button
                    ...
                    className={`text-xs px-3 py-1.5 rounded text-left font-bold transition-colors ${mapStatusFilter === "ALL" ? "bg-gray-700 text-white" : "text-gray-400 hover:text-gray-200"}`}
                  >
                    ALL UNITS
                  </button>
                  {Object.values(VehicleStatus).map((status) => (
                    <button
                      ...
                      className={`text-xs px-3 py-1.5 rounded text-left font-bold transition-colors flex items-center gap-2 ${mapStatusFilter === status ? "bg-gov-800 border border-gov-600 text-white" : "text-gray-400 hover:text-gray-200"}`}
                    >
                      <div
                        className={`w-2 h-2 rounded-full ${status === VehicleStatus.ON_MISSION ? "bg-sky-400" : status === VehicleStatus.ACTIVE ? "bg-emerald-400" : status === VehicleStatus.MAINTENANCE ? "bg-amber-400" : "bg-red-500"}`}
                      ></div>
                      {status.replace("_", " ")}
                    </button>
                  ))}
              ...
              <TacticalMap
                vehicles={vehicles.filter(
                  (v) =>
                    mapStatusFilter === "ALL" || v.status === mapStatusFilter,
                )}
                onVehicleSelect={handleVehicleSelect}
                selectedVehicleId={selectedVehicle?.id}
              />
              ...
              {incomingRequests.length > 0 && (
                <div className="bg-gov-800 rounded-lg border border-gov-700 p-4 shadow-lg border-l-4 border-l-gov-warning animate-in slide-in-from-top-4">
                  <h3 className="text-sm font-bold text-gray-300 flex items-center gap-2 uppercase tracking-wider mb-4">
                    <Bell
                      size={16}
                      className="text-gov-warning animate-pulse"
                    />{" "}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {incomingRequests.map((req) => (
                      <div
                        key={req.id}
                        className="bg-gov-900 p-4 rounded border border-gov-700 flex flex-col justify-between hover:border-gov-600 transition-colors"
                      >
                        <div>
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <span
                              className={`px-2 py-0.5 rounded text-[10px] font-bold ${req.priority === "VIP" ? "bg-amber-500/20 text-amber-500" : "bg-blue-500/20 text-blue-500"}`}
                            >
                              {req.priority}
                            </span>
                          </div>
                        ...
                        <div className="flex gap-2 mt-2">
                          <button
                            ...
                            className="flex-1 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 py-1.5 rounded text-xs font-bold transition-colors"
                          >
                            Approve
                          </button>
                          <button
                            ...
                            className="flex-1 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/30 py-1.5 rounded text-xs font-bold transition-colors"
                          >
                            Reject
                          </button>
                        </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm">
                    <thead className="bg-gov-900 text-gray-400 uppercase text-xs tracking-wider font-bold">
                      <tr>
                        <th className="p-4 border-b border-gov-700">
                          Mission ID
                        </th>
                        <th className="p-4 border-b border-gov-700">
                          Passenger
                        ...
                        <th className="p-4 border-b border-gov-700 text-right">
                          Actions
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gov-700">
                      {filteredMissions.map((trip) => {
                        ...
                        return (
                          <tr
                            key={trip.id}
                            className="hover:bg-gov-700/50 transition-colors"
                          >
                            <td className="p-4 font-mono text-gov-accent font-bold">
                              {trip.id}
                            </td>
                            ...
                            <td className="p-4">
                              <span
                                className={`px-2 py-0.5 rounded text-[10px] font-bold ${trip.priority === "VIP" ? "bg-amber-500/20 text-amber-500" : trip.priority === "URGENT" ? "bg-red-500/20 text-red-500" : "bg-blue-500/20 text-blue-500"}`}
                              >
                                {trip.priority}
                              </span>
                            </td>
                            ...
                            <td className="p-4 text-right">
                              <div className="flex justify-end gap-2">
                                <button
                                  ...
                                  className="p-1.5 text-gray-400 hover:text-white bg-gov-700 hover:bg-gov-600 rounded transition-colors"
                                  title="Edit Mission"
                                >
                                  <Edit2 size={14} />
                                </button>
                                {trip.status !== "COMPLETED" &&
                                  trip.status !== "CANCELLED" && (
                                    <button
                                      ...
                                      className="p-1.5 text-red-400 hover:text-red-300 bg-gov-700 hover:bg-red-900/50 rounded transition-colors"
            ...
            <div className="space-y-6">
                ...
                <div className="flex gap-2 w-full md:w-auto">
                  <div className="relative flex-1 md:w-64">
                    ...
                    <input
                      ...
                      className="w-full bg-gov-800 border border-gov-600 rounded-lg pl-10 pr-4 py-2 text-sm text-white focus:border-gov-accent outline-none"
                    />
                  </div>
                  <button className="bg-gov-800 hover:bg-gov-700 px-3 py-2 rounded border border-gov-600 text-gray-400">
                    <Filter size={18} />
                  </button>
                  <button
                    onClick={() => setShowAddVehicleModal(true)}
                    className="bg-gov-accent hover:bg-sky-600 px-4 py-2 rounded text-sm text-white font-bold whitespace-nowrap"
                  >
                    Add Vehicle
                  </button>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {filteredVehicles.map((vehicle) => (
                  <div
                    key={vehicle.id}
                    className="bg-gov-800 rounded-lg border border-gov-700 overflow-hidden hover:border-gov-accent/50 transition-colors group"
                  >
                    <div className="p-4 border-b border-gov-700 flex justify-between items-start bg-gov-900/50">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${vehicle.status === VehicleStatus.ACTIVE ? "bg-emerald-500/20 text-emerald-500" : vehicle.status === VehicleStatus.ON_MISSION ? "bg-sky-500/20 text-sky-500" : vehicle.status === VehicleStatus.MAINTENANCE ? "bg-amber-500/20 text-amber-500" : "bg-red-500/20 text-red-500"}`}
                      >
                        {vehicle.status.replace("_", " ")}
                      </span>
                    </div>
                    <div className="p-4 space-y-3">
                      <div className="flex justify-between items-center text-sm border-b border-gov-700/50 pb-2">
                        <div className="flex items-center gap-2 text-gray-400">
                          <Car size={14} /> Type
                        </div>
                        <span className="text-white text-xs bg-gov-700 px-1.5 py-0.5 rounded">
                          {vehicle.type}
                        </span>
                      </div>
                      <div className="flex justify-between items-center text-sm border-b border-gov-700/50 pb-2">
                        <div className="flex items-center gap-2 text-gray-400">
                          <FileText size={14} /> VIN
                        </div>
                      ...
                      <div className="flex justify-between items-center text-sm">
                        ...
                        <span className="text-white text-xs">
                          {vehicle.assignedDriverId
                            ? users.find(
                                (u) => u.id === vehicle.assignedDriverId,
                              )?.name || vehicle.assignedDriverId
                            : "Unassigned"}
                        </span>
                      </div>
                    </div>
                    <div className="p-3 bg-gov-900 border-t border-gov-700 flex justify-between opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className="text-xs flex items-center gap-1 text-gov-accent hover:text-white"
                      >
                        <LocateFixed size={12} /> Track
                      </button>
                      <button
          ...
          {activeTab === "CALENDAR" && (
            <CalendarView trips={trips} vehicles={vehicles} />
          )}

          {activeTab === "PERSONNEL" && (
            <PersonnelManager users={users} onUpdateUser={handleUserUpdate} />
          )}
          ...
          {activeTab === "MAINTENANCE" && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                ...
                <button className="bg-gov-700 hover:bg-gov-600 px-4 py-2 rounded text-sm text-white font-bold shadow-lg">
                  New Work Order
                </button>
              </div>
                  ...
                  <tbody className="divide-y divide-gov-700">
                    {trips
                      ...
                      .map((trip, idx) => (
                        <tr
                          key={trip.id}
                          className={`hover:bg-gov-700/50 transition-colors ${idx % 2 === 0 ? "bg-gov-800" : "bg-gov-800/50"}`}
                        >
                          <td className="p-4 font-mono text-gov-accent font-bold">
                            {trip.id}
                          </td>
                          ...
                          <td className="p-4">
                            <span
                              className={`px-2 py-0.5 rounded text-xs font-bold border ${trip.status === "COMPLETED" ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-500" : "bg-red-500/10 border-red-500/30 text-red-500"}`}
                            >
                              {trip.status}
                            </span>
                          </td>
          ...
          {activeTab === "SETTINGS" && (
            <div className="max-w-3xl space-y-6">
              ...
              <div className="bg-gov-800 border border-gov-700 rounded-lg p-6 space-y-6 shadow-lg">
                <div className="space-y-4">
                  <h3 className="text-sm font-bold text-gray-400 border-b border-gov-700 pb-2 uppercase tracking-wider">
                    Unit Identity
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSettings({ ...settings, unitName: e.target.value })
                        }
                        className="w-full bg-gov-900 border border-gov-600 rounded p-2 text-white focus:border-gov-accent outline-none"
                      />
                    </div>
                    <div className="space-y-1">
                      ...
                      <div className="relative">
                        <input
                          ...
                          onChange={(e) =>
                            setSettings({
                              ...settings,
                              fuelAlertThreshold: parseInt(e.target.value),
                            })
                          }
                          className="w-full bg-gov-900 border border-gov-600 rounded p-2 text-white focus:border-gov-accent outline-none"
                        />
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSettings({
                            ...settings,
                            refreshRateSeconds: parseInt(e.target.value),
                          })
                        }
                        className="flex-1 accent-gov-accent cursor-pointer"
                      />
                      <span className="text-white font-mono w-10 text-right">
                        {settings.refreshRateSeconds}s
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-4 flex justify-end">
                  <button
                    ...
                    className="bg-gov-success hover:bg-emerald-600 text-white px-6 py-2 rounded font-bold shadow-lg shadow-emerald-900/20 flex items-center gap-2 transition-transform active:scale-95"
                  >
        ...
        {showDispatchWizard && (
          <TripDispatchWizard
            drivers={users}
            vehicles={vehicles}
            onDispatch={handleCreateTrip}
            onCancel={() => setShowDispatchWizard(false)}
          />
        )}
        {showAddVehicleModal && (
          <AddVehicleModal
